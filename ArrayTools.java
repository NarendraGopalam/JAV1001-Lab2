...
public class ArrayTools {
	...
	public static void main (String args[]) //Start of the program
	{
		...
		to_be_encrypted=sc.nextLine(); //reads the string value to be encrypted from console
		//sc.nextLine();
		...
		System.out.println(decrypt(encrypted_data,shift_value)); //decrypted value
	...
	public static String decrypt(String to_be_decrypted,int shift_value) //method is a string type and contains two parameters which are a string and int types
	{
		String decrypted_data=""; //final out will be stored in this variable

		for(int index=0;index<to_be_decrypted.length();index++) //iterates until the end of stirng length
		{
			decrypted_data+=(char) (to_be_decrypted.charAt(index)-shift_value); // charAt used to get char value and shift value is cliculated and return ascii code and is converted to char
		}

		return decrypted_data; //returns final output
	}
